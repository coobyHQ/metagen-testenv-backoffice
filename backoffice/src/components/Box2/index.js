...
class Box2 extends Component {
	...
	save() {
		...
		if(this.validate(data)) {
			this.service.sendData(
				...
				(response)=> {
					...
					Utility.showModal({
						...
						subtitle: "Metadata successfully saved",
						text: "Clicking Ok will automatically download the saved metadata. To change the metadata, save it again by entering the same Entity ID",
						isOpen: true,
						callbackOk: ()=> {
							this.export();
				...
				(error)=> {
					...
					Utility.showModal({
						title: "Metadata Validation",
						subtitle: "There were errors when saving the metadata",
						text: error,
						isOpen: true,
					}); 
			...
			Utility.showModal({
				title: "Form compilation error",
				subtitle: "",
				text: "One or more fields in the form have not been completed correctly. ",
				isOpen: true
			});	
