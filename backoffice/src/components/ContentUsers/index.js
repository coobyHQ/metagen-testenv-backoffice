...
class ContentUsers extends Component {
	...
	setTaxCode(value) { this.setState({ taxCode: value}) }
	...
	setIDCardIssuer(value) { this.setState({ idCardIssuer: value}) }
	...
	save() {
		...
		service.saveUser({
			...
			(error)=> { 
				...
				Utility.showModal({
					title: "Saving user",
					subtitle: "Errors occurred while saving the user",
					text: error,
					isOpen: true
				}); 
			}
	...
	deleteUser(userName) {
		...
		Utility.showModal({
			...
			subtitle: "",
			text: "Sure you want to delete the user: " + userName + " ?",
			isOpen: true,
			callbackOk: ()=> {
				...
				service.deleteUser({
					...
					(error)=> { 
						...
						Utility.showModal({
							title: "Saving user",
							subtitle: "Errors occurred while deleting the user",
							text: error,
							isOpen: true
						}); 
					}
	...
	makeIdCard() {
		...
		if(this.state.idCardIssuer!="") idCardField.push(this.state.idCardIssuer);
